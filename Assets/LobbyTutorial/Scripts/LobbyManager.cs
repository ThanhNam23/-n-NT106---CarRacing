    ...
    private async void StartGame()
    {
        if (currentLobby != null && IsHost())
        {
            try
            {
                ...
                // Truyền thông tin người chơi qua NetworkManager
                GameData.Instance.playerNames = new List<string>();
                foreach (var player in currentLobby.Players)
                {
                    GameData.Instance.playerNames.Add(player.Data["PlayerName"].Value);
                }

                // Truyền thông tin người chơi qua NetworkManager
                GameData.Instance.playerName = PlayerPrefs.GetString("Name");
